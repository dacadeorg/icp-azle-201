...
if __name__ == "__main__":
  ...
  Frontend canister via browser""")
        for name, id in frontend_canisters:
            print(f"    {name}: {GREEN}{host}/?canisterId={id}{NORMAL}{BOLD}")
        print("  Backend canister via Candid interface:")
        for name, id in backend_canisters:
            print(f"    {name}: {GREEN}{host}/?canisterId={candid_ui_canister_id}&id={id}{NORMAL}{BOLD}")
        print(f"{NORMAL}", end="")
